...
func RunDygodMeijuSpider() {
	...
	for _, item := range items {
		...
		articleTag := models.ArticleTag{ArticleId: article.ID, TagId: 25}
...
func RunDygodMovieSpider() {
	...
	//http://www.dy2018.com/html/gndy/oumei/
	//http://www.dy2018.com/html/tv/oumeitv/index.html
	if err := page.Open("http://www.dy2018.com/html/gndy/oumei/"); err != nil {
		log.Fatal(err)
	}
	//注入js 文件必须在打开文件之后
	if err := page.InjectJS("jquery.min.js"); err != nil {
	...
	for _, item := range items {
		...
		var article models.Article
		models.Gorm.Where(models.Article{UrlProvider: url}).Assign(models.Article{RawTitle: title, UrlProvider: url, Body: content, RawContent: content, Title: title}).FirstOrCreate(&article)
		articleTag := models.ArticleTag{ArticleId: article.ID, TagId: 26}
		models.Gorm.Where(articleTag).Assign(articleTag).FirstOrCreate(&articleTag)
		//fmt.Println(article)
	}
