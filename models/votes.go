...
type Votes struct {
	...
	Visit     uint      `orm:"column(visit)"`
	Score     float64   `orm:"column(score);digits(4);decimals(2)"`
	VoteCount uint      `orm:"column(vote_count)"`
}
...
// UpdateVotes updates Votes by Id and returns error if
...
func DeleteVotes(id int) (err error) {
	o := orm.NewOrm()
	v := Votes{Id: id}
	// ascertain id exists in the database
	if err = o.Read(&v); err == nil {
		var num int64
		if num, err = o.Delete(&Votes{Id: id}); err == nil {
