...
type Images struct {
	...
	ArticleId   uint      `orm:"column(article_id)"`
	Description string    `orm:"column(description);size(255)"`
	Bucket      string    `orm:"column(bucket);size(255)"`
	Fname       string    `orm:"column(fname);size(255)"`
	Fsize       string    `orm:"column(fsize);size(255)"`
	CreatedAt   time.Time `orm:"column(created_at);type(timestamp);null"`
	UpdatedAt   time.Time `orm:"column(updated_at);type(timestamp);null"`
	Width       uint      `orm:"column(width)"`
	Height      uint      `orm:"column(height)"`
	Format      string    `orm:"column(format);size(20);null"`
}
